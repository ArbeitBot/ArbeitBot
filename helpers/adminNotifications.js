...
function sendNewReviewAlerts(bot, review) {
  let message = formNewReviewMessageText(review);

  admins.forEach(admin => {
    let keyboard = adminNewReviewKeyboard(review);
...
eventEmitter.on(strings.adminNotifications.adminBanReviewInline, ({ msg, bot }) => {
  let reviewId = msg.data.split(strings.inlineSeparator)[1];
...
function refreshAllAdminReviewMessages(review, bot) {
    ...
    let messages = review.inlineMessages;
    let text = formNewReviewMessageText(review);
    messages.forEach(messageInfo => {
      let data = messageInfo.split('+');
